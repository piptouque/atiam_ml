...
def plot_patterns(P, D):
    ...
    for i in range(nPats):
        c = Dcopy[i]
        ax.scatter(P[0, i], P[1, i], marker=symbols[c], c=np.array(
            colors[c]).reshape(1, -1), s=50, linewidths=2, edgecolor='w')
    # ax.legend()
    ax.grid(True)
    return fig
...
def plot_boundary(W, iVal, style, fig):
    ...
    for i in range(nUnits):
        ...
        plt.plot(xLims, (-np.dot(W[i, 1], xLims) - W[i, 0]) /
                 W[i, 2], linestyle=style, color=color, linewidth=1.5)
        fig.canvas.draw()
...
def visualize_boundary_linear(X, y, model):
    ...
    plt.scatter(X[pos, 0], X[pos, 1], marker='x',
                linewidths=2, s=23, color=[0, 0.5, 0])
    plt.scatter(X[neg, 0], X[neg, 1], marker='o',
                linewidths=2, s=23, color=[1, 0, 0])
    plt.plot(xp, yp, '-b')
    plt.scatter(model["X"][:, 0], model["X"][:, 1], marker='o',
                linewidths=4, s=40, color=None, edgecolors=[0.1, 0.1, 0.1])
...
def plot_data(X, y):
    ...
    plt.scatter(X[pos, 0], X[pos, 1], marker='x', edgecolor='k',
                linewidths=2, s=50, color=[0, 0.5, 0])
    plt.scatter(X[neg, 0], X[neg, 1], marker='o', edgecolor='k',
                linewidths=2, s=50, color=[1, 0, 0])
    return fig
...
def visualize_boundary(X, y, model):
    ...
    plt.scatter(model["X"][:, 0], model["X"][:, 1], marker='o',
                linewidths=4, s=10, color=[0.1, 0.1, 0.1])
...
def fit_multivariate_gaussian(X_s):
    ...
    pos[:, :, 0] = X
    pos[:, :, 1] = Y
    norm = distribution.MultivariateNormal(torch.Tensor(
        gmm.means_[0]), torch.Tensor(gmm.covariances_[0]))
...
def fit_gaussian_mixture(X_s):
    ...
    for i in range(4):
        norm = distribution.MultivariateNormal(torch.Tensor(
            gmm.means_[i]), torch.Tensor(gmm.covariances_[i]))
        Z += torch.exp(norm.log_prob(torch.Tensor(pos))).numpy()
    plt.figure(figsize=(10, 8))
    ax = plt.gca()
    cset = ax.contourf(X, Y, Z, cmap='magma')
...
def plot_gmm(gmm, X, label=True, ax=None):
    ...
    if label:
        ax.scatter(X[:, 0], X[:, 1], c=labels, s=40,
                   cmap='magma', edgecolor='gray', zorder=2)
    else:
